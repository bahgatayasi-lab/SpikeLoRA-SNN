...
The raw CSV files are not redistributed in this anonymized archive. Place the four source files in one directory using these exact names:
...
Use `data/R2_dataset_audit.csv` to verify SHA-256 hashes, date ranges, feature order, physical horizons, and supervised-window counts. A hash mismatch means the input file is not byte-identical to the data used for the reported R2 results.
